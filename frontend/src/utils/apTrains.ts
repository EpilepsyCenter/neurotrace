import type { APPoint } from '../stores/appStore'
 ...
 *  action potentials, walking sweep by sweep so trains never span
 *  two trials. Sorts each sweep's spikes by ``peakT`` before calling
 ...
 *  agree.
 *
 *  Twin of ``utils/burstTrains.ts`` and ``utils/eventTrains.ts`` —
 *  same algorithm, different input shape (APPoint has ``sweep`` +
 *  ``peakT`` and is purely point-event, no extent).
 *
 *  Returns ``idByGlobalIdx`` of length ``spikes.length`` (``null`` =
 *  isolated) and ``flatSummaries`` annotated with the sweep each
 *  train belongs to. */
export function computeApTrainIds(
  spikes: APPoint[],
  ...
  for (const sw of sortedSweeps) {
    ...
    sweepIdxs.sort((a, b) => spikes[a].peakT - spikes[b].peakT)
